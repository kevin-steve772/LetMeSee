...
# Increase the maximum file descriptors if we have reason.
if [ "$cygwin" = "false" -a "$darwin" = "false" -a "$nonstop" = "false" ] ; then
    MAX_FD_LIMIT=`ulimit -H -n`
    if [ "$MAX_FD_LIMIT" != "" ] ; then
        if [ "$MAX_FD_LIMIT" -lt 256 ] ; then
            echo "Could not set maximum file descriptor limit: $MAX_FD_LIMIT"
        fi
    fi
fi

# For Darwin, add options to specify how the java command should be launched.
if [ "$darwin" = "true" ] ; then
